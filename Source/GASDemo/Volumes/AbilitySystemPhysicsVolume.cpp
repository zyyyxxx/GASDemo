...
void AAbilitySystemPhysicsVolume::ActorEnteredVolume(AActor* Other)
{
	...
	if(UAbilitySystemComponent* AbilitySystemComponent = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(Other))
	{
		...
		for(auto EventTag : GameplayEventsToSendOnEnter)
		{
			...
			if(!HasAuthority())
			{
				ServerProxySendGameplayEventToActor(Other , EventTag , EventPayload);
			}
		}
	}
}
...
void AAbilitySystemPhysicsVolume::Tick(float DeltaSeconds)
{
	...
	if(bDrawDebug)
	{
		DrawDebugBox(GetWorld() , GetActorLocation() , GetBounds().BoxExtent , FColor::Red ,
			false, 0 , 0 ,5);
	}
}

void AAbilitySystemPhysicsVolume::ServerProxySendGameplayEventToActor_Implementation(AActor* TargetActor,
